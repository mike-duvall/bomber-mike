...
BlitterObject * BombPowerup::theArchetypeBlitterObject_ = 0;

BombPowerup::BombPowerup(int x, int y ) : Powerup(x,y)
{
...
void BombPowerup::Activate(Player * aPlayer)
{
	aPlayer->IncrementNumberOfAllowedBombs();
}



void BombPowerup::SetupArchetype()
{
	int blockWidth = 32;
	int blockHeight = 32;
	...
	{ 
		char filename[80];
		// build up file name
		sprintf(filename,"bitmaps/BombPowerup.BMP",direction);
