...
_asNode * CAStar::FindBestPartialPathEndNode()
{
	_asNode * fittestNode = m_pClosed;
	_asNode * currentNode = m_pClosed;

	while(currentNode->next != 0)
	{
		if(currentNode->next->h < fittestNode->h)
		{
			fittestNode = currentNode->next;
		}
...
_asNode * CAStar::FindNodeAtCoordinates(int x, int y)
{
	int nodeNumber = Coord2Num(x,y);

	_asNode * foundNode = CheckList(m_pClosed, nodeNumber);
	if(foundNode != 0)
	{
		return foundNode;
	}
	...
	{
		return CheckList(m_pOpen, nodeNumber);
	}
