...
int dd_pixel_format = DD_PIXEL_FORMAT888;
...
int Draw_BOB_To_Relative_Coordinates(BOB_PTR bob,               // bob to draw
			 LPDIRECTDRAWSURFACE7 dest, int xBase, int yBase) // surface to draw the bob on
{
	// draw a bob at the x,y defined in the BOB
	// on the destination surface defined in dest
	...
	// fill in the destination rect
	dest_rect.left   = bob->x + xBase;
	dest_rect.top    = bob->y + yBase;
	dest_rect.right  = dest_rect.left + bob->width;
	dest_rect.bottom = dest_rect.top + bob->height;
				   ...
				   // cx,cy are absolute coords
{
	...
	LPDIRECTDRAWSURFACE7 bobSurface = bob->images[frame];
	...
	source_rect.left = cx;
	source_rect.top = cy;
	source_rect.right = cx + bob->width;
	source_rect.bottom = cy + bob->height;

	dest_rect.left = 0;
	dest_rect.top = 0;
	dest_rect.right = bob->width;
	dest_rect.bottom = bob->height;
...
int Draw_Text_GDI(const char *text, int x,int y,int color, TCHAR * szFaceName, int iDeciPtHeight,LPDIRECTDRAWSURFACE7 lpdds)
{
	// this function draws the sent text on the sent surface 
	// using color index as the color in the palette
...
//	static LOGFONT lf;  // structure for font information  
	...
	HFONT font = EzCreateFont (xdc, szFaceName, iDeciPtHeight, 0,0, TRUE);

	HGDIOBJ oldFont = SelectObject(xdc, font);

	// draw the text a
	TextOut(xdc,x,y,text,(int)strlen(text));

	SelectObject(xdc, oldFont);
	DeleteObject(font);
...
if (bitmap->bitmapinfoheader.biBitCount==24)
   ...
   for (index=0; index < bitmap->bitmapinfoheader.biWidth*bitmap->bitmapinfoheader.biHeight; index++)
	   ...
	   if (dd_pixel_format==DD_PIXEL_FORMATALPHA888 )
		   {
...
//       ((USHORT *)bitmap->buffer)[index] = color;
	   (bitmap->buffer)[index] = (UCHAR)color;
			 ...
			 int lpitch)
{
	...
	UINT *vbuffer2 = (UINT *)vbuffer; // short pointer to buffer

	// convert pitch to words
	//	lpitch = lpitch >> 1;
	// convert pitch to quads
	lpitch = lpitch >> 2;
	...
	UINT color = _RGB32BIT(alpha, red, green, blue);
	// draw the row of pixels
	Mem_Set_QUAD((vbuffer2+(y*lpitch)+x1), color,x2-x1+1);


}

///////////////////////////////////////////////////////////
void VLine32(int y1,int y2,int x,int alpha, int red, int green, int blue,UCHAR *vbuffer, int lpitch)
{
	...
	UINT *start_offset; // starting memory offset of line

	int index, // loop index
		temp;  // used for temporary storage during swap

	// convert lpitch to number of quads
	lpitch = lpitch >> 2;
	...
	start_offset = (UINT *)vbuffer + (y1*lpitch) + x;
...
bool Collision_Test_Mike_Fix(int x1, int y1, int w1, int h1, 
...
if (poly->state)
   ...
   for (index=0; index < poly->num_verts-1; index++)
        ...
        Draw_Clip_Line16(int(poly->vlist[index].x+poly->x0), 
       ...
       Draw_Clip_Line16(int(poly->vlist[0].x+poly->x0), 
...
if (poly->state)
   ...
   for (index=0; index < poly->num_verts-1; index++)
        {
        // draw line from ith to ith+1 vertex
        Draw_Clip_Line(int(poly->vlist[index].x+poly->x0), 
                       int(poly->vlist[index].y+poly->y0),
                       int(poly->vlist[index+1].x+poly->x0), 
                       int(poly->vlist[index+1].y+poly->y0),
                       poly->color,
       ...
       // now close up polygon
       // draw line from last vertex to 0th
       Draw_Clip_Line(int(poly->vlist[0].x+poly->x0), 
                      int(poly->vlist[0].y+poly->y0),
                      int(poly->vlist[index].x+poly->x0), 
                      int(poly->vlist[index].y+poly->y0),
                      poly->color,
...
Mat_Init_3X2(&mt,1,0, 0,1,(float) dx, (float)dy); 
