...
TitleGameState::TitleGameState()
{
	...
	blackPlayer = new SimpleGameObject(titleX + 10 ,titleY - 172,122,146,"IntroPlayerBlack.bmp");
	whitePlayer = new SimpleGameObject(640 + 140 ,titleY + 212,178,198,"IntroPlayerWhite.bmp");
	title = 0;
	yearTitle = 0;
	clonedBy = 0;
	mikeDuVall = 0;
...
void TitleGameState::HandleTitleMoving()
{
	...
	{
		titleMovingTimer = 50;
		title = new SimpleGameObject(titleX,titleY,512,448,"MainTitle.bmp");
		bomberTitle = new SimpleGameObject(600,75,274,116,"BomberTitle.bmp",_RGB32BIT(0,1,217,138));
		manTitle = new SimpleGameObject(730,140,186,72,"ManTitle.bmp",_RGB32BIT(0,1,217,138));
...
void TitleGameState::HandleAnimatingCredits()
{
	if(yearTitle == 0)
	{
		yearTitle = new SimpleGameObject(375,90,154,126,"93-Title.bmp",_RGB32BIT(0,1,217,138));
	}
	...
	if(clonedBy == 0)
	{
		clonedBy = new SimpleGameObject(700,400,87,24,"Cloned by.bmp",_RGB32BIT(0,36,0,109));
		cloneByTimer = 33;
	...
	else
	{
		if(mikeDuVall == 0)
		{
			mikeDuVall = new SimpleGameObject(-150,425,169,24,"Mike DuVall.bmp",_RGB32BIT(0,36,0,109));
			mikeDuVallTimer = 26;
			Sound::PlaySound(Sound::creditsWhoosh, .5f);
		}
...
void TitleGameState::HandleAnimatingFashizzle()
{
	...
	if(pushButton == 0)
	{
	pushButton = new SimpleGameObject(190,280,192,16,"PushButton.bmp",_RGB32BIT(0,36,0,109));
	}
	...
	else
	{
		if(fashizzle == 0)
		{
		fashizzle = new SimpleGameObject(-30,425,63,10,"Fashizzle.bmp",_RGB32BIT(0,36,0,109));
		}
